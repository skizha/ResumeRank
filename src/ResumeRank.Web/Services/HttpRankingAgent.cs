...
public class HttpRankingAgent : IRankingAgent
{
    private readonly HttpClient _httpClient;
    private readonly ILogger<HttpRankingAgent> _logger;

    public HttpRankingAgent(HttpClient httpClient, ILogger<HttpRankingAgent> logger)
    {
    ...
    {
        var request = new RankAgentRequest
        {
        ...
        try
        {
            ...
            var result = await response.Content.ReadFromJsonAsync<RankAgentResponse>();
            if (result == null)
                throw new InvalidOperationException("Ranking agent returned null response");
        ...
        {
            _logger.LogError(ex, "Failed to call ranking agent");
            throw;
        }
        catch (TaskCanceledException ex)
        {
            _logger.LogError(ex, "Timeout calling ranking agent");
            throw;
        }
    }

    private class RankAgentRequest
    {
        [JsonPropertyName("resumes")]
        public List<RankAgentResume> Resumes { get; set; } = new();

        [JsonPropertyName("job")]
        public RankAgentJob Job { get; set; } = null!;
    }

    private class RankAgentResume
    {
    ...
    private class RankAgentResponse
    {
        [JsonPropertyName("rankings")]
        public List<RankAgentScore> Rankings { get; set; } = new();
    }

    private class RankAgentScore
    {
        [JsonPropertyName("resume_id")]
        public int ResumeId { get; set; }
